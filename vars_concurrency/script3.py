## THIS SCRIPT WILL INHERIT DATA FROM HOST FILE, GROUP FILE AND DEFAULT FILE. DEFAULT IS LOW, GROUP IS DEFAULT AND HOST FILE WILL BE HIGH PREFERENCE 

import yaml
from inventory import DEVICES
from rich import print as rprint


def load_vars(device, groups):
    device_name = device["device_name"]
    data_dict = {}
    default_data = yaml.safe_load(open("group_vars/defaults.yaml"))
    if default_data is not None:
        for k,v in default_data.items():
            data_dict[k] = v
    all_group_data = []
    for group in groups:
        group_data = yaml.safe_load(open(f"group_vars/{group}.yaml"))
        if group_data is not None:
            all_group_data.append(group_data)
    for group_data in all_group_data:
        for k,v in group_data.items():
            data_dict[k] = v
    host_data = yaml.safe_load(open(f"host_vars/{device_name}.yaml"))
    if host_data is not None:
        for k,v in host_data.items():
            data_dict[k] = v
    print(data_dict)

def main():
    for device in DEVICES:
        groups = device["groups"]
        deviace_vars = load_vars(device, groups)

main()